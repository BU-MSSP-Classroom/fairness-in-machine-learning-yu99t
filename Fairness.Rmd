...
A German financial company wants to create a model that predicts the defaults on consumer loans in the German market.  When you are called in, the company has already built a model and they ask you to evaluate the model because there is a concern raised that this model unfairly evaluates the young customers.  Your task is to figure out if this is true or not and to come up with a way to correct this problem.  The data used to make prediction as well as the predictions can be found in `germancredit` data.
...
In US, judges, probation officers, and parole officers are using algorithms to evaluate the likelihood of a criminal defendant re-offending, a concept commonly referred to as recidivism. Numerous risk assessment algorithms are currently in circulation with two prominent nationwide tools provided by commercial vendors.

One of these tools, Northpointe's COMPAS (Correctional Offender Management Profiling for Alternative Sanctions), has made national headlines about how it seems to have bias towards certain protected groups. Your job is to figure out if this is the case.
...
Fairness metrics have several ways to classify them.  Many of the fairlness metrics for discrete outcomes are derived using the conditional confusion matrix.   For each of the protected groups of interest, we can define a conditional confusion matrix as:
...
Proportional parity is very similar to demographic parity but modifies it to address the issue that when the number of cases within each group is different, which artificially boosts the disparity. In some literature, proportional parity and demographic parity are considered equivalent, which is true when the protected group size are equivalent.  Proportional parity is achieved if the proportion of positive predictions in the subgroups are close to each other. Similar to the demographic parity, this measure also does not depend on the true labels.
...
Specificity parity can be considered the ‘inverse’ of the equalized odds.  Specificity are calculated by the division of true negatives with all negatives (irrespective of predicted values). 
$$TN_g / (TN_g + FP_g)$$  
 ...
 In the returned named vector, the reference group will be assigned 1, while all other groups will be assigned values according to whether their specificity are lower or higher compared to the reference group. Lower specificity will be reflected in numbers lower than 1 in the returned named vector, thus numbers lower than 1 mean WORSE prediction for the subgroup.
...
There are handfull of software that has been made available over the last few years.  These are usually a combination of fairness metrics calculation, followed by visualizations.
...
We will look at the `germancredit` data again.  But here we will create our own model.  As a comparison, let's fit logistic regression and random forest model.
